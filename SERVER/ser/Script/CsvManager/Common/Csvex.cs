...
using System;
    ...
    public class CsvEx
    {
        ...
        private bool CheckCSV()
        {
            ...
            for (int i = 0, j = m_dt.Count - 1; i < j; i++, j--)
            {
                if (m_dt[i].Count != m_dt[j].Count)
                {
                    ...
                    //Debug.LogWarning(string.Format("第{0}行：列数{1}，内容:{2}", j, m_dt[j].Count, sb));
        ...
        private string[] m_strArrCName;         // 第二行
        private string[] m_strArrEName;         // 第一行
        private string m_strSaveData;
        private eTypes[] m_IndexType = null;    // 第三行
    };
    //
