...
namespace Roma
{
    public class UIPanelJoyStick : UIBase
    {
        ...
        {
            base.Init();
            //m_effectCam = m_root.FindChild("camera").GetComponent<Camera>();
            //m_effectCam.orthographicSize = Screen.height * 0.5f;
            m_move = m_root.FindChild("panel/dynamic/move").gameObject.AddComponent<MoveJoyStick>();
            ...
            {
                m_SkillBtn[i] = skillParent.FindChild(i.ToString()).gameObject.AddComponent<SkillJoyStick>();
                m_SkillBtn[i].Init();
            }
