...
namespace Roma
{
    ...
    public class CBHAStar
    {
        ...
        public bool LineTest2(Map map, int x1, int y1, int x2, int y2, ref Vector2 lastPos)
        {          
        ...
        public bool LineTest3(Map map, int startX, int startY, int endX, int endY, ref List<Vector2> paths)
        {
            // k = (y1-y2) / (x1-x2)
            int deltaX = endX - startX; // 最大增量
            int deltaY = endY - startY;
            int unitX = (deltaX > 0) ? 1 : -1;  // 方向单元格
            int unitY = (deltaY > 0) ? 1 : -1;
            ...
            if (Mathf.Abs(deltaX) > Mathf.Abs(deltaY))   // 如果向左大于向前
            {
                for (curX = startX; curX != endX; curX += unitX)
                {
                    curY = (int)(k * (curX - startX)) + startY;
                    if (!map.CanArriveDoubleInt(curX, curY))
                    {
                        return false;
                    }
                    paths.Add(new Vector2(curX, curY));
                }
            }
            else
            {
                for (curY = startY; curY != endY; curY += unitY)
                {
                    curX = (int)((float)(curY - startY)/k) + startX;
                    if (!map.CanArriveDoubleInt(curX, curY))
                    {
                        return false;
                    }
        ...
        /// <param name="x2"></param>
        /// <param name="y2"></param>
        /// <param name="mpaths">返回直线上，从起点到终点，能走动的所有点</param>
        /// <returns></returns>
        public bool LineTest2(Map map, int x1, int y1, int x2, int y2, ref List<Vector2> mpaths)
        {
            //Profiler.BeginSample("LineTest2");
            int dx = x2 - x1;
            int dy = y2 - y1;
        ...
        public bool FindPath(Map map, ref Vector2 startPt, ref Vector2 endPos, ref List<Vector2> paths, int maxRoute)
        {
            // 如果检测到是能走的直线，就取最后一个点为目的地
            // 如果检测到不是能走的直线，取靠近目标点最近的并且能走的点，作为寻路的目标点
            Vector2 vLastPos = Vector2.zero;
            if (LineTest2(map, (int)startPt.x, (int)startPt.y, (int)endPos.x, (int)endPos.y, ref vLastPos))
            {
                paths.Add(vLastPos);
                return true;
            }
            ...
            if (null == map || !map.CanArriveDoubleInt(ref endPos))
            {
                m_nErrorType = eErrorCode.eErrParam;
           ...
           // sPaths.Clear();
            ...
            while (m_nOpenCount > 0)
            {
                ...
                if (++currTry > maxRoute)
                {
                    m_nErrorType = eErrorCode.eErrMaxTry;
                ...
                getArounds(map, currNoteX, currNoteY, ref aroundNodes, ref aroundCount);
            ...
            // 开放列表已空，找不到路径
            m_nErrorType = eErrorCode.eErrCannotFind;
        ...
        // 取得失败原因
        eErrorCode GetErrorType() { return m_nErrorType; }
        ...
        // 判断某节点是否在开放列表
        bool isOpen(Map map, int p_x, int p_y)
        {
        ...
        bool getArounds(Map map, int p_x, int p_y, ref Vector2[] aroundNodes, ref int aroundCount)
        {
            ...
            checkX = p_x + 1;
            checkY = p_y;
            canRight = map.CanArriveDoubleInt(checkX, checkY);
            ...
            checkX = p_x;
            checkY = p_y + 1;
            canDown = map.CanArriveDoubleInt(checkX, checkY);
            ...
            checkX = p_x - 1;
            checkY = p_y;
            canLeft = map.CanArriveDoubleInt(checkX, checkY);
            ...
            checkX = p_x;
            checkY = p_y - 1;
            canUp = map.CanArriveDoubleInt(checkX, checkY);
            ...
            checkX = p_x + 1;
            checkY = p_y + 1;
            canDiagonal = map.CanArriveDoubleInt(checkX, checkY);
