Shader "Roma/Model/Dissolve" {
	Properties{
		...
		fixed _Tile;             // 平铺值  
		fixed _Amount;           // 溶解度  
		fixed _DissSize;         // 溶解范围  
		fixed4 _DissColor;       // 溶解颜色  
		fixed4 _AddColor;        // 叠加色  
								// 最终色  
		static fixed3 finalColor = fixed3(1,1,1);

		struct Input {
			fixed2 uv_MainTex;  // 只需要主材质的UV信息  
		};
		...
		void surf(Input IN, inout SurfaceOutput o) {
			...
			// 对裁剪材质进行采样，取R色值  
			fixed ClipTex = tex2D(_DissolveText, IN.uv_MainTex / _Tile).r;
