...
Shader "FGame/static/ms/cutedgeheightlight"
{
	Properties
	{
	...
	{	
		...
		Pass
		{
			...
			{
				//CameraDepthNormalsTexture（深度图get）
				half screenDepth = DecodeFloatRG(tex2D(_CameraDepthNormalsTexture, i.screenuv).zw);
				half diff = screenDepth - i.depth;
				...
				half glow = max(max(intersect, rim), northPole);
				fixed4 SecondCol = tex2D(_SecondTex, i.uv1) * _Color;

				fixed glowColorAlpha = 1;
				//fixed4 glowColor = fixed4(lerp(_Color.rgb, fixed3(1, 1, 1), pow(glow, 2)), saturate(sin(glowColorAlpha)*_Time.y));
				fixed4 glowColor = fixed4(lerp(_Color.rgb, fixed3(1, 1, 1) * SecondCol * 12 , pow(glow, 2)), 1) ;
				
				
				fixed4 hexes = texColor(i, rim);
				
				//fixed4 col =glowColor * glow + hexes;
